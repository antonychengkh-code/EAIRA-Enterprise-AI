...
The B2-MAN-006 Stage 1 documentary option-selection questions identified in its decision record are selected as recorded below. All local executable, version, configuration, fsmonitor, index-write, environment, non-mutation, output, Field 8, and Field 9 verification dependencies remain unresolved. Unrelated Stage 1 details remain unselected. Field 2 and Field 3 remain `PARTIALLY_RESOLVED_WITH_REQUIRED_REVISIONS`; the four substantive blockers remain exactly `B2-MAN-006`, `B2-MAN-007`, `B2-MAN-010`, and `B2-MAN-013`; and the gate remains `BLOCKED`. No mutation authority beyond this documentary Annex synchronization, Field-state change, gate-state change, Gate B, inspection, execution, or evidence-collection authority is granted.
...
| `B2-MAN-007` | Four tab-separated fields: commit SHA, author name, ISO-8601 author timestamp, subject. | 30 seconds | `NONE` | Planning-input approval remains unchanged, but this row remains a substantive documentary blocker. Partial-clone or promisor uncertainty may cause lazy object retrieval and resulting network and credential-helper pathways. Pager and configuration behavior and existing Field 9 handling of author name and commit subject remain unresolved. The row remains non-executable. | `PROPOSED_PERSON_NAME_OUTPUT_PENDING_FIELDS_8_AND_9` | `REP-B2-007` | `MAP-B2-007` | `APPROVED_AS_MANIFEST_PLANNING_INPUT` |
...
| `B2-MAN-007` | Stop if email, commit body, signature material, or more than four fields appears; if partial-clone or promisor status is unresolved; if lazy object retrieval or resulting network or credential-helper activity can occur; if pager or configuration behavior is unresolved; or if existing Field 9 author-name or subject requirements are unmet. | Author name and subject require Field 9 review; email/body are prohibited. | `EVIDENCE_PATH_PENDING_FIELD_8_APPROVAL/B2-MAN-007.txt` | `APPROVED_AS_MANIFEST_PLANNING_INPUT` |
...
| `REP-B2-007` | SHA, author name, ISO-8601 author time, subject. | Author-name and subject decisions. | `EAIRA_EVIDENCE_ROOT/direct-output/<session>__B2-MAN-007__REP-B2-007.pending` | Stop on email, body, signature, or extra fields; unresolved partial-clone or promisor status; possible lazy object retrieval or resulting network or credential-helper activity; unresolved pager or configuration behavior; or unmet Field 9 author-name or subject requirements. | `MAP-B2-007` | `APPROVED_AS_REPRODUCTION_PLANNING_CONTROL_NOT_EXECUTABLE` |
...
| `MAP-B2-007` | `B2-MAN-007` | Observe one commit SHA, author name, author timestamp, and subject. | Field 8 direct/redacted output for `REP-B2-007`. | Verify four fields, redaction decisions, absence of email/body, partial-clone or promisor status, lazy-fetch behavior and resulting network or credential-helper pathways, pager and configuration behavior, and existing Field 9 author-name and subject handling. | Exactly four expected fields safely captured only after the adopted lazy-fetch, pager/configuration, and existing Field 9 dependencies are resolved under separate authorization. |
...
1. Independently verify the complete Version 0.20.0 Annex, exact changed-file list, and exact path-scoped diff. Field 1 and Field 4 documentary review remains recorded as complete by the separately adopted Field-State Decision; the Version 0.20.0 synchronization records the B2-MAN-006 Stage 1 documentary detail selections while preserving the historical/current logical command, the separate non-executable future envelope, every Field state, the four substantive blockers, and the `BLOCKED` gate.

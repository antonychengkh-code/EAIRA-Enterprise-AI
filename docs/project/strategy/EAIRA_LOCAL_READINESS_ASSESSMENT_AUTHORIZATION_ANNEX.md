...
6. Follow the future lifecycle order: preserve every previously published Main Annex version as historical Annex publication evidence; treat the version identified in Section 1 metadata of the Main Annex in the repository commit under independent verification as the current publication, with that publication commit and the Main Annex blob contained by it as the binding current-publication identities; independently verify the complete current publication, including its exact changed-file list, exact path-scoped diff, commit, parent, Main Annex blob, and remote publication; obtain a separate Project Owner decision before any downstream impact review; obtain a separate later Project Owner authorization before any downstream synchronization; retain local-verification authorization-package preparation as separately unauthorized; and treat executable discovery, inspection, option testing, command execution, evidence collection, criterion evaluation, blocker decisions, Field review, and gate evaluation as separate later stages requiring their own Project Owner authority.
...
1. Treat the version identified in Section 1 metadata of the Main Annex in the repository commit under independent verification as the current Annex publication, with that publication commit and the Main Annex blob contained by it as the binding current-publication identities.
2. Independently verify the complete current Annex publication, including its exact changed-file list, exact path-scoped diff, commit, parent, Main Annex blob, and remote publication.
3. Obtain a separate Project Owner decision before any downstream impact review.
4. Obtain a separate later Project Owner authorization before any downstream synchronization; publication and verification do not authorize it automatically.
5. Treat local-verification authorization-package preparation, executable discovery, inspection, option testing, command execution, evidence collection, criterion evaluation, blocker decisions, Field review, and gate evaluation as later separate stages requiring their own Project Owner authority.

No downstream impact review or downstream synchronization is completed or authorized by publication or independent verification alone. Route C execution-package preparation, Route C execution, conditional Route B matched official-source review, any Field review, and any gate evaluation remain later separate matters. Any future Field and execution lifecycle must retain the existing sequence: separate Field-state reviews for Fields 2, 3, 8, 9, 10, and 11; separate formal all-fields-resolved gate evaluation; separate assessment-execution authorization if the gate passes; only then exact authorized execution and evidence collection; independent evidence verification; bounded `INITIAL_MINIMAL_CORE_SCOPE_READINESS` conclusion; and a later separate Project Owner decision.

...
6. Follow the future lifecycle order: preserve every previously published Main Annex version as historical Annex publication evidence; treat B2-MAN-006 local-verification authorization-package preparation, Project Owner package disposition, canonical freeze, repository decision-record publication, bounded independent remote exact-ref verification, and the response-only Main Annex impact review as completed documentary stages; treat the version identified in Section 1 metadata of the Main Annex in the repository commit under independent verification as the current publication, with that publication commit and the Main Annex blob contained by it as the binding current-publication identities; independently verify the complete current publication, including its exact changed-file list, exact path-scoped diff, commit, parent where available, Main Annex blob, and remote publication; obtain a separate Project Owner decision before any downstream impact review; obtain a separate later Project Owner authorization before any downstream synchronization; and treat local executable discovery, local inspection, actual verification, option testing, command execution, evidence collection, criterion evaluation, blocker decisions, Field review, and gate evaluation as separate later stages requiring their own Project Owner authority.
...
The shared WSL Git executable-binding package and its adoption decision are repository-recorded, and the B2-MAN-006 local-verification authorization package is now separately adopted with inline canonical-text preservation only as bounded planning evidence. The shared requirement count is `22`, with satisfied count `0`; the shared category count is `20`, with satisfied count `0`; B2-MAN-006 satisfied category count remains `0` of `6`; and B2-MAN-007 satisfied category count remains `0` of `16`. B2-MAN-006 and B2-MAN-007 remain unresolved substantive blockers, no Field or gate state changes, and assessment execution remains unauthorized.

## 19. Future Review Requirement

The required immediate documentary lifecycle sequence is:

1. Treat B2-MAN-006 local-verification authorization-package preparation, Project Owner package disposition, canonical freeze, repository decision-record publication, bounded independent remote exact-ref verification, response-only Main Annex impact review, and this Version 0.27.0 synchronization as completed documentary stages only.
2. Treat Version 0.27.0 as the current Annex publication only after independent repository verification establishes the publication commit, exact one-file changed set, exact path-scoped diff, Main Annex blob, and remote publication. Parent identity must be reported if the available connector returns it; absence of parent evidence must remain an explicit gap rather than being inferred.
...
5. Treat local executable discovery, local inspection, actual verification, option testing, command execution, evidence collection, criterion evaluation, blocker disposition, Field review, and gate evaluation as separate later stages requiring their own Project Owner authority.

The bounded independent verification of the B2-MAN-006 adoption record retains the recorded default-branch, parent-chain, direct-compare, canonical-hash-recomputation, and local-state gaps. This synchronization does not close those gaps and does not convert remote exact-ref evidence into local working-tree, local HEAD, or local-to-remote synchronization evidence.

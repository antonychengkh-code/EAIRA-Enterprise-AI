...
| `B2-MAN-010` | `TGT-DOCKER-001` | WSL CLI; WSL path | `docker` | `docker --version` | `--version` | `/mnt/c/Users/User/OneDrive/文件/EAIRA-Enterprise-AI` | Codex under Antony Cheng's explicit task-level authorization | `NO_ELEVATION` | Obtain Docker client product, version, and build text only; no intentional Docker Engine query is permitted. |
...
| `B2-MAN-010` | One Docker client version string and build identifier if emitted. | 30 seconds | `NONE` | Planning-input approval remains unchanged, but this row remains a substantive documentary blocker. Docker client product, version, and build intent is documented and no intentional Docker Engine query is permitted. Version-scoped CLI initialization, configuration loading, supported plugin or extension loading, supported credential handling, and possible pre-exit Engine contact remain unresolved. Bounded output cannot prove absence of Engine contact. The row remains non-executable. | `PROPOSED_DIRECT_OUTPUT_PENDING_FIELDS_8_AND_9` | `REP-B2-010` | `MAP-B2-010` | `APPROVED_AS_MANIFEST_PLANNING_INPUT` |
...
| `B2-MAN-010` | Stop if an intentional Docker Engine query is requested, Engine contact occurs, output exceeds Docker client product/version/build text, the applicable Docker version or primary documentation is unresolved, or version-scoped CLI initialization, configuration loading, supported plugin or extension loading, or supported credential handling remains unresolved. | None expected; reject unexpected fields. | `EVIDENCE_PATH_PENDING_FIELD_8_APPROVAL/B2-MAN-010.txt` | `APPROVED_AS_MANIFEST_PLANNING_INPUT` |
...
| `REP-B2-010` | Docker client product/version/build only. | Client-metadata retention approval. | `EAIRA_EVIDENCE_ROOT/direct-output/<session>__B2-MAN-010__REP-B2-010.pending` | Stop on Engine contact, any intentional Engine query, output beyond Docker client product/version/build text, unresolved Docker version or primary documentation, or unresolved version-scoped CLI initialization, configuration loading, supported plugin or extension loading, or supported credential handling. | `MAP-B2-010` | `APPROVED_AS_REPRODUCTION_PLANNING_CONTROL_NOT_EXECUTABLE` |
...
| `MAP-B2-010` | `B2-MAN-010` | Observe Docker client product/version/build text only. | Field 8 direct output for `REP-B2-010`. | Verify bounded Docker client product/version/build output and the applicable version-scoped primary documentation; separately verify CLI initialization, configuration loading, supported plugin or extension loading, supported credential handling, and Engine-contact behavior because bounded output alone cannot prove absence of Engine contact. | Bounded Docker client product/version/build text with the adopted Docker-specific initialization and Engine-contact uncertainties resolved under separate authorization. |

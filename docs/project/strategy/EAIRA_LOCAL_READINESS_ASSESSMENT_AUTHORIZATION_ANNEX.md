...
All other Stage 1 details remain unselected. Field 2 and Field 3 remain `PARTIALLY_RESOLVED_WITH_REQUIRED_REVISIONS`; the four substantive blockers remain exactly `B2-MAN-006`, `B2-MAN-007`, `B2-MAN-010`, and `B2-MAN-013`; and the gate remains `BLOCKED`. No mutation, Field-state change, gate-state change, Gate B, inspection, execution, or evidence-collection authority is granted.
...
| `B2-MAN-006` | Branch header and zero or more tracked repository-relative status entries. | 30 seconds | `NONE` | Planning-input approval remains unchanged, but this row remains a substantive documentary blocker. Two distinct unresolved chains apply: (1) possible index refresh or write, optional-lock behavior, and post-index-change behavior; and (2) fsmonitor daemon or hook behavior and possible fsmonitor socket or state creation. Applicable repository-discovery and executable-binding dependencies also remain unresolved. The row remains non-executable. | `PROPOSED_PATH_BEARING_OUTPUT_PENDING_FIELDS_8_AND_9` | `REP-B2-006` | `MAP-B2-006` | `APPROVED_AS_MANIFEST_PLANNING_INPUT` |
...
| `B2-MAN-006` | Stop if untracked entries, absolute paths, or data outside tracked status fields appear; if possible index refresh or write, optional-lock behavior, or post-index-change behavior is uncontrolled; if fsmonitor daemon or hook behavior or fsmonitor socket or state creation is uncontrolled; or if applicable repository-discovery or executable-binding dependencies remain unresolved. | Repository-relative paths require Field 9 review; reject sensitive or absolute paths. | `EVIDENCE_PATH_PENDING_FIELD_8_APPROVAL/B2-MAN-006.txt` | `APPROVED_AS_MANIFEST_PLANNING_INPUT` |
...
| `REP-B2-006` | Branch header and tracked relative status entries only. | Repository-relative-path decisions. | `EAIRA_EVIDENCE_ROOT/direct-output/<session>__B2-MAN-006__REP-B2-006.pending` | Stop on untracked, absolute, sensitive, or unrelated fields; uncontrolled possible index refresh or write, optional-lock, or post-index-change behavior; uncontrolled fsmonitor daemon or hook behavior or fsmonitor socket or state creation; or unresolved repository-discovery or executable-binding dependencies. | `MAP-B2-006` | `APPROVED_AS_REPRODUCTION_PLANNING_CONTROL_NOT_EXECUTABLE` |
...
| `MAP-B2-006` | `B2-MAN-006` | Observe branch header and tracked working-tree status with untracked enumeration disabled. | Field 8 direct/redacted output for `REP-B2-006`. | Verify options, tracked-only scope, path handling, provenance, the optional-lock/index-write/post-index-change chain, the separate fsmonitor daemon-or-hook/socket-or-state chain, and the mapped repository-discovery and executable-binding dependencies. | Bounded status output containing only permitted fields, with both distinct risk chains and the mapped repository-discovery and executable-binding dependencies resolved under separate authorization. |
...
2. Field 1 target-inventory review and Field 4 interaction-inventory review are complete for mandatory Annex pre-execution planning completeness. For Fields 2 and 3, retain the seven supported or broadly supported rows and the selected Area 8 Option 8a documentary Object-Name Expectation Rule. Resolve B2-MAN-006, B2-MAN-007, B2-MAN-010, and B2-MAN-013, together with the still-unselected common Git invocation envelope, exact executable binding, repository object format through future separately authorized repository-object-format verification, and existing Field 8 and Field 9 dependencies, without inferring execution authority.
...
1. Independently verify the complete Version 0.17.0 Annex, exact changed-file list, and exact path-scoped diff. Field 1 and Field 4 documentary review remains recorded as complete by the separately adopted Field-State Decision; the Version 0.17.0 correction changes Field 2 semantics wording only and does not reclassify any Field.

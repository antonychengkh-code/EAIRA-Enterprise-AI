...
| 3 | Command-specific arguments, working directory, privilege, target, purpose, expected output, timeout, and mutation-risk determination | `PARTIALLY_RESOLVED_WITH_REQUIRED_REVISIONS` | Retained command-specific planning controls remain subject to evidence, redaction, semantic-verification, satisfaction, and existing Field 8 and Field 9 dependencies. Remaining semantic work is limited to the adopted row-specific controls for B2-MAN-006, B2-MAN-007, B2-MAN-010, and B2-MAN-013 and the unselected common Git-envelope and object-name details applicable to supported or broadly supported rows. Field 3 remains PARTIALLY_RESOLVED_WITH_REQUIRED_REVISIONS. |
...
| `B2-MAN-004` | One full `HEAD` object name whose form and width conform to the verified repository object format and the separately selected Object-Name Expectation Rule. | 30 seconds | `NONE` | Local `HEAD` resolution is supported as documentary semantics planning evidence. The common Git invocation envelope, exact executable binding, repository object format, and Object-Name Expectation Rule remain unresolved. Planning-input approval and non-executable status remain unchanged; no execution is authorized. | `PROPOSED_DIRECT_OUTPUT_PENDING_FIELDS_8_AND_9` | `REP-B2-004` | `MAP-B2-004` | `APPROVED_AS_MANIFEST_PLANNING_INPUT` |
| `B2-MAN-005` | One full object name resolving only the local `origin/master` remote-tracking ref, with form and width governed by the verified repository object format and separately selected Object-Name Expectation Rule; it does not prove live-remote existence, equality, or freshness. | 30 seconds | `NONE` | Local `origin/master` resolution is supported as documentary semantics planning evidence and requests no fetch or live-remote inquiry. The common Git invocation envelope, exact executable binding, repository object format, and Object-Name Expectation Rule remain unresolved. Planning-input approval and non-executable status remain unchanged; no live-remote freshness or execution authority is established. | `PROPOSED_DIRECT_OUTPUT_PENDING_FIELDS_8_AND_9` | `REP-B2-005` | `MAP-B2-005` | `APPROVED_AS_MANIFEST_PLANNING_INPUT` |
...
| `B2-MAN-004` | Stop if output is not one full object name conforming to the verified repository object format and selected Object-Name Expectation Rule, if the common Git invocation envelope or exact executable binding is unresolved, or if additional output appears. | None expected; reject unexpected fields. | `EVIDENCE_PATH_PENDING_FIELD_8_APPROVAL/B2-MAN-004.txt` | `APPROVED_AS_MANIFEST_PLANNING_INPUT` |
| `B2-MAN-005` | Stop if output is not one full object name for the local `origin/master` ref conforming to the verified repository object format and selected Object-Name Expectation Rule, if a fetch, live-remote inquiry, or other network action is attempted, or if the common Git invocation envelope or exact executable binding is unresolved. | None expected; reject unexpected fields. | `EVIDENCE_PATH_PENDING_FIELD_8_APPROVAL/B2-MAN-005.txt` | `APPROVED_AS_MANIFEST_PLANNING_INPUT` |
...
| `REP-B2-004` | One full `HEAD` object name conforming to the verified repository object format and selected Object-Name Expectation Rule. | Exact-SHA retention approval. | `EAIRA_EVIDENCE_ROOT/direct-output/<session>__B2-MAN-004__REP-B2-004.pending` | Stop if the output is not one full object name conforming to the verified repository object format and selected Object-Name Expectation Rule, if additional metadata appears, or if the common Git envelope or executable binding is unresolved. | `MAP-B2-004` | `APPROVED_AS_REPRODUCTION_PLANNING_CONTROL_NOT_EXECUTABLE` |
| `REP-B2-005` | One full object name resolving only the local `origin/master` ref and conforming to the verified repository object format and selected Object-Name Expectation Rule; no live-remote existence, equality, or freshness inference. | Exact-SHA retention approval. | `EAIRA_EVIDENCE_ROOT/direct-output/<session>__B2-MAN-005__REP-B2-005.pending` | No fetch, live-remote inquiry, or network action; stop if output is not one full local-ref object name conforming to the verified repository object format and selected Object-Name Expectation Rule, if additional metadata appears, or if the common Git envelope or executable binding is unresolved. | `MAP-B2-005` | `APPROVED_AS_REPRODUCTION_PLANNING_CONTROL_NOT_EXECUTABLE` |
...
| `MAP-B2-004` | `B2-MAN-004` | Observe one full current `HEAD` object name whose form and width conform to the verified repository object format and selected Object-Name Expectation Rule. | Field 8 direct output for `REP-B2-004`. | Verify one full object name against the established repository object format and selected Object-Name Expectation Rule, together with provenance. | One object name conforming to the verified repository object format and selected Object-Name Expectation Rule, captured under valid authorization and the resolved common Git envelope. |
| `MAP-B2-005` | `B2-MAN-005` | Observe one full object name resolving only the existing local `origin/master` remote-tracking ref without fetch, live-remote inquiry, or network access; do not infer live-remote existence, equality, or freshness. | Field 8 direct output for `REP-B2-005`. | Verify one local-ref object name against the established repository object format and selected Object-Name Expectation Rule; verify no fetch, live-remote inquiry, or network action; verify provenance. | One well-formed local remote-tracking object name conforming to the verified repository object format and selected Object-Name Expectation Rule, with no live-remote inference. |
...
2. Field 1 target-inventory review and Field 4 interaction-inventory review are complete for mandatory Annex pre-execution planning completeness. For Fields 2 and 3, retain the seven supported or broadly supported command rows and resolve only the adopted row-specific controls for B2-MAN-006, B2-MAN-007, B2-MAN-010, and B2-MAN-013, together with the unselected common Git-envelope, executable-binding, repository object-format, Object-Name Expectation Rule, and existing Field 8 and Field 9 dependencies, without inferring executable availability, environment readiness, criterion satisfaction, or execution authority.

...
6. Follow the future lifecycle order: preserve Versions 0.16.0 through 0.21.0 as historical evidence; independently verify the complete Version 0.22.0 Annex and exact diff; obtain separate Project Owner authorization before downstream Stage 3, local Git-for-Windows/WSL identification, conditional Route B review, or any later Field-state review; then follow the existing separate Field-review, formal gate-review, execution-authorization, authorized execution/evidence, independent-verification, bounded-conclusion, and later-decision sequence.
...
1. Independently verify the complete Version 0.22.0 Annex, exact changed-file list, and exact path-scoped diff. Version 0.22.0 records the corrected B2-MAN-013 accepted-review identifier, documentary route, matched-source and WSL strategy, retained historical/current command, separate future verified-identity representation, and fail-closed controls while preserving every Field state, the four substantive blockers, and the `BLOCKED` gate. Obtain separate Project Owner authorization before downstream Stage 3, Route C local identification, or conditional Route B review.

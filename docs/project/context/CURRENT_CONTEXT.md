...
- M4 Slice 4 Gate 24 initial and R1 independent reviews returned `CANNOT_CLOSE`; D08 profile calibration found one P1 and R2R1 moved mandatory external profile SHA validation before JSON parsing. The later R2R1 sealed-final review returned `CANNOT_CLOSE`, `P0=0`, `P1=2`, `P2=0`, for omitted named abuse coverage and a stale allowlist state. R2R2 adds the complete directory/file attribute-and-tag cross-matrix at all stages, unrelated physical eighth-file non-enumeration, Unicode scalar/whitespace behavior, explicit NUL/CR/tab rejection and zero-result raw-query non-emission while retaining exact per-output raw IL binding, A/B stability, token-relocation-aware semantic equality and 17 compiled-and-rejected specimens. The bound state is phase-stable across the authorized Gate 24-28 lifecycle and does not claim a new reviewed discovery or later transition. [Clause S4-STATE-ACTIVE-LINKS]
...
The current product lifecycle is `EAIRA_M4_SLICE4_GATE24_TO_GATE28_CONTROLLED_LIFECYCLE`: independently reviewed profile calibration, a new empty-root sealed A/B run, full independent Gate 24 review, exact 21-path staging and review, normal commit and verification, then normal push and independent live-publication verification. Discovery manifest `9D7215AD311E4CF5518E2164F9CF2571F04C928338B590FB3F2B55F6DB125F54` is provisional only. Checkpoint-ref repair, Windows, service, signing and production mutations remain unauthorized.
...
| Active task | `docs/project/status/ACTIVE_TASK.yaml`, `docs/project/strategy/EAIRA_M4_FUNCTIONAL_AGENT_MVP_SLICE_4_SCOPE_DECISION.md`, `docs/project/planning/EAIRA_M4_SLICE4_BOUNDED_READ_ONLY_PROJECT_KNOWLEDGE_QUERY_ALLOWLIST.md`, `docs/project/planning/EAIRA_M4_SLICE4_BOUNDED_READ_ONLY_PROJECT_KNOWLEDGE_QUERY_THREAT_MODEL.md`, and `docs/project/planning/EAIRA_M4_SLICE4_BOUNDED_READ_ONLY_PROJECT_KNOWLEDGE_QUERY_READINESS_PACKAGE.md` |

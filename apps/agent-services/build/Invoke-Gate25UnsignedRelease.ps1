...
$operatorDiscovery = $LocalOperatorDiscovery -or $LocalOperatorPreflightDiscovery
$modeCount = @($DevelopmentProbe,$ProjectKnowledgeDiscovery,$ProjectQaDiscovery,$LocalOperatorDiscovery,$LocalOperatorPreflightDiscovery | Where-Object { [bool]$_ }).Count
...
$expectedOperatorCapabilities = @('TASK','KNOWLEDGE','PROJECT_QA','HEALTH','PREFLIGHT')
...
if ($profile.localOperator.contract -ne 'EAIRA_LOCAL_OPERATOR_V1' -or $profile.localOperator.revision -ne 3 -or
    ...
    $profile.localOperator.expectedHarnessTests -ne 166 -or $profile.localOperator.expectedHarnessCanonicalNameBytes -ne 4783 -or
    [string]$profile.localOperator.expectedHarnessCaseNameSha256 -cne '02CD503C883C8929CD7BA393BEBAFBBB932CDBA5DB22348F298FF66E9E182595' -or
...
if ($profileOperatorInputs.Count -ne 34 -or ((@($profileOperatorInputs | ForEach-Object { [string]$_.file })) -join "`n") -cne ($localOperatorBoundPaths -join "`n")) {
    throw 'Local Operator profile must bind the exact ordered 34 inputs.'
}
...
foreach ($relativePath in $localOperatorBoundPaths) {
    ...
    if (-not $operatorDiscovery -and -not $DevelopmentProbe -and ($null -eq $expected -or [string]$expected.sha256 -cne $actualHash)) {
...
for ($buildIndex = 0; $buildIndex -lt $buildRoots.Count; $buildIndex++) {
    ...
    $operatorFrameText = 'EAIRA_M5_SLICE3_CASE_NAMES_V1' + [char]0
    ...
    foreach ($name in @($actualLocalOperatorCases | Select-Object -Skip 119)) { $operatorSlice3NewFrameText += $name.Length.ToString([Globalization.CultureInfo]::InvariantCulture) + ':' + $name }
    ...
    $localOperatorHarnessPass = $localOperatorHarnessTest.exitCode -eq 0 -and $localOperatorHarnessTest.stderrBytes.Length -eq 0 -and
        ...
        [int]$localOperatorHarnessJson.testsPassed -eq 166 -and (($actualLocalOperatorCases -join "`n") -ceq ($expectedLocalOperatorCases -join "`n")) -and
        $operatorFrameBytes.Length -eq 4783 -and $operatorFrameSha256 -ceq '02CD503C883C8929CD7BA393BEBAFBBB932CDBA5DB22348F298FF66E9E182595' -and
    ...
    if (-not $operatorDiscovery) { Assert-LoopbackMetadataPolicy -LiteralPath $localOperatorOutputPath -Policy $profile.localOperator.loopbackMetadataAllowlist -DevelopmentProbe:$DevelopmentProbe }
    ...
    try { $operatorInvalidJson=[Text.Encoding]::UTF8.GetString($operatorInvalid.stdoutBytes)|ConvertFrom-Json; $operatorDeniedJson=[Text.Encoding]::UTF8.GetString($operatorDenied.stdoutBytes)|ConvertFrom-Json; $operatorMockJson=[Text.Encoding]::UTF8.GetString($operatorMock.stdoutBytes)|ConvertFrom-Json; $operatorHealthJson=[Text.Encoding]::UTF8.GetString($operatorHealth.stdoutBytes)|ConvertFrom-Json; $operatorPreflightJson=[Text.Encoding]::UTF8.GetString($operatorPreflight.stdoutBytes)|ConvertFrom-Json } catch { throw 'Local Operator CLI output is not valid JSON.' }
    ...
    if(-not$operatorDiscovery-and(($preflightGoldenVectorEvidence|ConvertTo-Json -Depth 6 -Compress)-cne($profile.localOperator.preflightGoldenVectors|ConvertTo-Json -Depth 6 -Compress))){throw 'Local Operator Preflight golden vector profile mismatch.'}
    ...
    foreach ($channel in @($operatorInvalid,$operatorDenied,$operatorMock,$operatorHealth,$operatorPreflight)) {
...
$localOperatorNativeProfileMatch = $operatorDiscovery -or (($allBuildEvidence[0].localOperatorCli.normalizedNativeInventory|ConvertTo-Json -Depth 8 -Compress) -ceq ($profile.localOperator.nativeCallerInventory|ConvertTo-Json -Depth 8 -Compress))
$localOperatorTestsPass = @($allBuildEvidence|Where-Object{-not $_.localOperatorHarness.offlineTestsPass -or -not $_.localOperatorCli.offlineTestsPass}).Count -eq 0 -and $localOperatorMetadataStable -and $localOperatorNativeStable -and $localOperatorLoopbackStable -and $localOperatorHealthIlStable -and $localOperatorPreflightIlStable -and $localOperatorHealthInstrumentationStable -and $localOperatorNegativeSpecimensPass -and $localOperatorNativeProfileMatch
...
$finalEvidence = -not $DevelopmentProbe -and -not $ProjectKnowledgeDiscovery -and -not $ProjectQaDiscovery -and -not $operatorDiscovery -and -not [String]::IsNullOrEmpty($ExpectedReleaseProfileSha256) -and $m4TechnicalChecksPass
$releaseOutputs = @()
...
$manifest = [ordered]@{
    ...
    classification = if ($DevelopmentProbe) { 'DEVELOPMENT_PROBE_ONLY' } elseif ($LocalOperatorPreflightDiscovery) { 'M5_SLICE3_OPERATOR_PREFLIGHT_DISCOVERY' } elseif ($LocalOperatorDiscovery) { 'M5_SLICE2_OPERATOR_HEALTH_DISCOVERY' } elseif ($ProjectQaDiscovery) { 'M4_SLICE_5_PROJECT_QA_DISCOVERY' } elseif ($ProjectKnowledgeDiscovery) { 'M4_SLICE_4_PROJECT_KNOWLEDGE_DISCOVERY' } else { 'M5_SLICE3_OPERATOR_PREFLIGHT_UNSIGNED_CANDIDATE' }
    status = if ($m4TechnicalChecksPass) { 'M5_SLICE3_UNSIGNED_TECHNICAL_CHECKS_PASS' } elseif ($DevelopmentProbe -and -not $reproducible) { 'BLOCKED_NONDETERMINISTIC_COMPILER' } else { 'FAIL_CLOSED' }
    ...
    localOperator = [ordered]@{
        ...
        normalizedNativeProfileMatch = if($operatorDiscovery){$null}else{[bool]$localOperatorNativeProfileMatch}

...
namespace EAIRA.AgentServices.Functional
{
    internal static class LocalOperatorHarness
    {
        ...
        private static void RunCoreCases()
        {
            ...
            for(int i=41;i<Names.Length;i++){int index=i;Case(Names[index],delegate{Exercise(index,h,p);});}
        }
        ...
        {
            string value="EAIRA_M5_SLICE1_CASE_NAMES_V1\0";foreach(string n in Passed)value+=ContractCodec.Field(n);bytes=Encoding.UTF8.GetByteCount(value);return ContractCodec.Sha256Hex(value);
        }
        internal static int Main(string[] args)
        {
            try
            {
                ...
                if(args==null||args.Length!=1||args[0]!="--self-test")return 64;RunCoreCases();Require(Passed.Count==96);int framed;string digest=Framed(out framed);Require(framed==2409&&digest=="0AAF52EE6A087B487B6497676F48BA7AD0D36CB987C149CC1BADAD9CA5D2FE68");
                Console.Write("{\"schema\":\"EAIRA_LOCAL_OPERATOR_HARNESS_V1\",\"status\":\"PASS\",\"testsPassed\":96,\"caseNames\":"+CaseJson()+",\"caseNameFramedBytes\":2409,\"caseNameSha256\":\""+digest+"\",\"wrapperMaximumBytes\":587,\"network\":\"NONE\",\"writes\":\"NONE\"}\n");return 0;
            }
